...
{	
	public partial class BuWFProcessDao : IDataAccess
	{
		const bool isDebug = true;
        private static readonly string DalSql = " Select ProcessId,processName from BuWFProcess Where 1=1 ";
   		     
   		private int RunCommandWithTransatcion(AYJZ.Entities.BuWFProcess ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" Insert Into BuWFProcess (");
            ...
            return RunCommandWithTransatcion((BuWFProcess)ent, insSQL.ToString(), TRANS);
        }
        ...
        {
            StringBuilder s_UpdSQL = new StringBuilder(" Update BuWFProcess Set ");
            bool isFirstValue = true;
            ...
            s_UpdSQL.Append(" WHERE ProcessId = @ProcessId");
            return RunCommandWithTransatcion((BuWFProcess)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        {
            List<BuWFProcess> list = new List<BuWFProcess>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   BuWFProcess ent = new BuWFProcess();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(BuWFProcess ent, IDataReader dr)
        {
			ent.ProcessId = MyConvert.ToInt(dr["ProcessId"]);
			ent.processName = MyConvert.ToString(dr["processName"]);
		}
