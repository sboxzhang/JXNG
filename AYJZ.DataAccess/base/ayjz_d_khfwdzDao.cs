...
{
    public partial class ayjz_d_khfwdzDao:IDataAccess
    {
        const bool isDebug = true;
        private static readonly string DalSql = "SELECT ID,EIID,KHFWDZ,XM,XB,SFZH,HYZK FROM ayjz_d_khfwdz where 1=1 ";
        
    
        private int RunCommandWithTransatcion(ayjz_d_khfwdzInfo ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" INSERT INTO ayjz_d_khfwdz (");
            ...
            return RunCommandWithTransatcion((ayjz_d_khfwdzInfo)ent, insSQL.ToString(), TRANS);
        }

        public int Delete(BaseEntitie ent, IDbTransaction TRANS)
        {
            string s_DelSQL = " DELETE FROM ayjz_d_khfwdz   WHERE  ID=@ID ";
            return RunCommandWithTransatcion((ayjz_d_khfwdzInfo)ent, s_DelSQL, TRANS);
        }

        public int Update(BaseEntitie ent, IDbTransaction TRANS)
        {
            StringBuilder s_UpdSQL = new StringBuilder(" UPDATE ayjz_d_khfwdz SET ");
            bool isFirstValue = true;
            ...
            return RunCommandWithTransatcion((ayjz_d_khfwdzInfo)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        /// <returns></returns>
        public ayjz_d_khfwdzInfo Getayjz_d_khfwdz( long ID)
        {
              ayjz_d_khfwdzInfo ent = null;
            string sql = DalSql;
            sql = sql + " And  ID=@ID ";
        ...
        /// <param name="ent"></param>
        /// <returns></returns>
        public List<ayjz_d_khfwdzInfo> Getayjz_d_khfwdzList(string Where)
        {
            List<ayjz_d_khfwdzInfo> list = new List<ayjz_d_khfwdzInfo>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   ayjz_d_khfwdzInfo ent = new ayjz_d_khfwdzInfo();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(ayjz_d_khfwdzInfo ent, IDataReader dr)
        {
            ent.ID = MyConvert.ToLong(dr["ID"]);
            ent.EIID = MyConvert.ToLong(dr["EIID"]);
            ent.KHFWDZ = MyConvert.ToString(dr["KHFWDZ"]);
