...
{
    public partial class ayjz_htxx_spDao:IDataAccess
    {
        const bool isDebug = true;
        private static readonly string DalSql = "SELECT ID,EIID,SFZ,LXDH,DZ,KSSJ,JSSJ,GZXX,DJ,AYID,NL,YBGZ,BZ,HTZT,SFSP,SPID,SPJG,CZLX FROM ayjz_htxx_sp where 1=1 ";
        
    
        private int RunCommandWithTransatcion(ayjz_htxx_spInfo ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" INSERT INTO ayjz_htxx_sp (");
            ...
            return RunCommandWithTransatcion((ayjz_htxx_spInfo)ent, insSQL.ToString(), TRANS);
        }

        public int Delete(BaseEntitie ent, IDbTransaction TRANS)
        {
            string s_DelSQL = " DELETE FROM ayjz_htxx_sp   WHERE  SPID=@SPID ";
            return RunCommandWithTransatcion((ayjz_htxx_spInfo)ent, s_DelSQL, TRANS);
        }

        public int Update(BaseEntitie ent, IDbTransaction TRANS)
        {
            StringBuilder s_UpdSQL = new StringBuilder(" UPDATE ayjz_htxx_sp SET ");
            bool isFirstValue = true;
            ...
            return RunCommandWithTransatcion((ayjz_htxx_spInfo)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        /// <returns></returns>
        public ayjz_htxx_spInfo Getayjz_htxx_sp( long SPID)
        {
              ayjz_htxx_spInfo ent = null;
            string sql = DalSql;
        ...
        /// <param name="ent"></param>
        /// <returns></returns>
        public List<ayjz_htxx_spInfo> Getayjz_htxx_spList(string Where)
        {
            List<ayjz_htxx_spInfo> list = new List<ayjz_htxx_spInfo>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   ayjz_htxx_spInfo ent = new ayjz_htxx_spInfo();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(ayjz_htxx_spInfo ent, IDataReader dr)
        {
            ent.ID = MyConvert.ToLong(dr["ID"]);
            ent.EIID = MyConvert.ToLong(dr["EIID"]);
            ent.SFZ = MyConvert.ToString(dr["SFZ"]);
