...
{
    public partial class ayjz_htjsDao:IDataAccess
    {
        const bool isDebug = true;
        private static readonly string DalSql = "SELECT ID,HTID,JSR,ZJE,JSSJ,SFSP FROM ayjz_htjs where 1=1 ";
        
    
        private int RunCommandWithTransatcion(ayjz_htjsInfo ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" INSERT INTO ayjz_htjs (");
            ...
            return RunCommandWithTransatcion((ayjz_htjsInfo)ent, insSQL.ToString(), TRANS);
        }

        public int Delete(BaseEntitie ent, IDbTransaction TRANS)
        {
            string s_DelSQL = " DELETE FROM ayjz_htjs   WHERE  ID=@ID ";
            return RunCommandWithTransatcion((ayjz_htjsInfo)ent, s_DelSQL, TRANS);
        }

        public int Update(BaseEntitie ent, IDbTransaction TRANS)
        {
            StringBuilder s_UpdSQL = new StringBuilder(" UPDATE ayjz_htjs SET ");
            bool isFirstValue = true;
            ...
            return RunCommandWithTransatcion((ayjz_htjsInfo)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        /// <returns></returns>
        public ayjz_htjsInfo Getayjz_htjs( long ID)
        {
              ayjz_htjsInfo ent = null;
            string sql = DalSql;
            sql = sql + " And  ID=@ID ";
        ...
        /// <param name="ent"></param>
        /// <returns></returns>
        public List<ayjz_htjsInfo> Getayjz_htjsList(string Where)
        {
            List<ayjz_htjsInfo> list = new List<ayjz_htjsInfo>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   ayjz_htjsInfo ent = new ayjz_htjsInfo();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(ayjz_htjsInfo ent, IDataReader dr)
        {
            ent.ID = MyConvert.ToLong(dr["ID"]);
            ent.HTID = MyConvert.ToLong(dr["HTID"]);
            ent.JSR = MyConvert.ToString(dr["JSR"]);
            ent.ZJE = MyConvert.ToDecimal(dr["ZJE"]);
