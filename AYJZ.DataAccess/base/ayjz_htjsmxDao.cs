...
{
    public partial class ayjz_htjsmxDao:IDataAccess
    {
        const bool isDebug = true;
        private static readonly string DalSql = "SELECT ID,SJ,JE,JSID FROM ayjz_htjsmx where 1=1 ";
        
    
        private int RunCommandWithTransatcion(ayjz_htjsmxInfo ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" INSERT INTO ayjz_htjsmx (");
            ...
            return RunCommandWithTransatcion((ayjz_htjsmxInfo)ent, insSQL.ToString(), TRANS);
        }

        public int Delete(BaseEntitie ent, IDbTransaction TRANS)
        {
            string s_DelSQL = " DELETE FROM ayjz_htjsmx   WHERE  ID=@ID ";
            return RunCommandWithTransatcion((ayjz_htjsmxInfo)ent, s_DelSQL, TRANS);
        }

        public int Update(BaseEntitie ent, IDbTransaction TRANS)
        {
            StringBuilder s_UpdSQL = new StringBuilder(" UPDATE ayjz_htjsmx SET ");
            bool isFirstValue = true;
            ...
            return RunCommandWithTransatcion((ayjz_htjsmxInfo)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        /// <returns></returns>
        public ayjz_htjsmxInfo Getayjz_htjsmx( long ID)
        {
              ayjz_htjsmxInfo ent = null;
            string sql = DalSql;
            sql = sql + " And  ID=@ID ";
        ...
        /// <param name="ent"></param>
        /// <returns></returns>
        public List<ayjz_htjsmxInfo> Getayjz_htjsmxList(string Where)
        {
            List<ayjz_htjsmxInfo> list = new List<ayjz_htjsmxInfo>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   ayjz_htjsmxInfo ent = new ayjz_htjsmxInfo();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(ayjz_htjsmxInfo ent, IDataReader dr)
        {
            ent.ID = MyConvert.ToLong(dr["ID"]);
            ent.SJ = MyConvert.ToString(dr["SJ"]);
            ent.JE = MyConvert.ToDecimal(dr["JE"]);
            ent.JSID = MyConvert.ToLong(dr["JSID"]);
        }
