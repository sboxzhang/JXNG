...
{
    public partial class ayjz_d_yhzhDao:IDataAccess
    {
        const bool isDebug = true;
        private static readonly string DalSql = "SELECT ID,EIID,YHZH,KHH,LX FROM ayjz_d_yhzh where 1=1 ";
        
    
        private int RunCommandWithTransatcion(ayjz_d_yhzhInfo ent, string vSql, IDbTransaction TRANS)
        {
            if (null == TRANS)
            {
        ...
        {
            StringBuilder insSQL = new StringBuilder(" INSERT INTO ayjz_d_yhzh (");
            ...
            return RunCommandWithTransatcion((ayjz_d_yhzhInfo)ent, insSQL.ToString(), TRANS);
        }

        public int Delete(BaseEntitie ent, IDbTransaction TRANS)
        {
            string s_DelSQL = " DELETE FROM ayjz_d_yhzh   WHERE  ID=@ID ";
            return RunCommandWithTransatcion((ayjz_d_yhzhInfo)ent, s_DelSQL, TRANS);
        }

        public int Update(BaseEntitie ent, IDbTransaction TRANS)
        {
            StringBuilder s_UpdSQL = new StringBuilder(" UPDATE ayjz_d_yhzh SET ");
            bool isFirstValue = true;
            ...
            return RunCommandWithTransatcion((ayjz_d_yhzhInfo)ent, s_UpdSQL.ToString(), TRANS);
        }
        ...
        /// <returns></returns>
        public ayjz_d_yhzhInfo Getayjz_d_yhzh( long ID)
        {
              ayjz_d_yhzhInfo ent = null;
            string sql = DalSql;
            sql = sql + " And  ID=@ID ";
        ...
        /// <param name="ent"></param>
        /// <returns></returns>
        public List<ayjz_d_yhzhInfo> Getayjz_d_yhzhList(string Where)
        {
            List<ayjz_d_yhzhInfo> list = new List<ayjz_d_yhzhInfo>();
            using(DbDataReader reader = DataBaseManage.ExecuteReader(DalSql+Where))
			{
                while (reader.Read())
                {
                   ayjz_d_yhzhInfo ent = new ayjz_d_yhzhInfo();
                    SetEnt(ent, reader);
                    list.Add(ent);
                }
            }
        ...
        public void SetEnt(ayjz_d_yhzhInfo ent, IDataReader dr)
        {
            ent.ID = MyConvert.ToLong(dr["ID"]);
            ent.EIID = MyConvert.ToLong(dr["EIID"]);
            ent.YHZH = MyConvert.ToString(dr["YHZH"]);
            ent.KHH = MyConvert.ToString(dr["KHH"]);
